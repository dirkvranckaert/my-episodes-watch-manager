...
public class EpisodesService {
	...
	public HttpClient getHttpClient() {
		...
		httpClient.getParams().setParameter(CoreProtocolPNames.USER_AGENT, "Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.36");
		return httpClient;
	}
    ...
    private StringWriter downloadFullUnwatched(HttpClient httpClient, User user, boolean isWatched) throws LoginFailedException, ShowUpdateFailedException, InternetConnectivityException, UnsupportedHttpPostEncodingException{
    	...
    	try {
    		...
    		if (entity != null) {
    			...
    			//read html file.
    			int startTable = HTMLtoDecode.indexOf("<table class=\"mylist\" width=\"100%\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\">") +78+202;
    	...
    	if (status != 200) {
    		String message = "Error downloading and processing, failed with status code " + status + " for URL " + urlRep;
    		Log.w(LOG_TAG, message);
    		throw new ShowUpdateFailedException(message);
    	} else {
